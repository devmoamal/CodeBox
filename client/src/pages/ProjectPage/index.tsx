...
export function ProjectPage() {
  ...
  return (
    <div className="h-screen w-screen flex flex-col bg-bg font-sans overflow-hidden select-none text-text transition-colors duration-300">
      <header className="h-14 border-b border-border/30 flex items-center justify-between px-6 bg-panel/80 backdrop-blur-md shrink-0 relative z-20">
        ...
        <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 flex items-center gap-3 pointer-events-none transition-all duration-300">
          {activeFilePath && (
            <div className="flex items-center gap-2 bg-bg/50 px-3 py-1 rounded-full border border-border/30 backdrop-blur-md">
              <span className="text-[10px] font-mono text-text-muted">/</span>
              <span className="text-[10px] font-mono text-primary-hover font-medium tracking-tight">
                {activeFilePath}
              </span>
            </div>
          )}
